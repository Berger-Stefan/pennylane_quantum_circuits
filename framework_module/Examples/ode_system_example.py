...
def analytical_fnc(t_eval):
    return analytical_sol_fnc.sol(t_eval)
...
def pde_res_fnc(model, t_eval):
    u_pred_1, u_pred_2 = model.forward(t_eval)

    grad_outputs_1 = torch.ones_like(u_pred_1)
    du_1_dt = torch.autograd.grad(u_pred_1, t_eval, grad_outputs=grad_outputs_1, create_graph=True)[0]
    grad_outputs_2 = torch.ones_like(u_pred_2)
    du_2_dt = torch.autograd.grad(u_pred_2, t_eval, grad_outputs=grad_outputs_2, create_graph=True)[0]
