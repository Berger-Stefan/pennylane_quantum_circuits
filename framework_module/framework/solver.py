...
class Solver:
    def __init__(
        ...
        model: Model,
        pde_res_fnc,
        boundary_res_fnc: list,
        loss_scaling=None,
        plot_update_functions: list = [],
    ) -> None:
        self.data = data
        self.model = model
    ...
    def loss_fnc(self):
        ...
        total_loss = self.loss_scaling[0] * pde_loss_value + sum(
            [self.loss_scaling[idx + 1] * val for idx, val in enumerate(boundary_losses)]
        )
        ...
        self.loss_values["boundary_loss"].append(
            sum([self.loss_scaling[idx + 1] * val.detach().numpy() for idx, val in enumerate(boundary_losses)])
        )
        ...
        if self.model.analytical_fnc != None:
            if self.model.output_dim > 1:
                ...
                for i in range(self.model.output_dim):
                    res_analytical += np.mean(
                        (
                            self.model.forward(self.data.domain)[i].detach().numpy()
                            - self.model.analytical_fnc(self.data.domain.detach().numpy())[i]
                        )
                        ** 2
                    )
            else:
                res_analytical = np.mean(
                    (
                        self.model.forward(self.data.domain).detach().numpy()
                        - self.model.analytical_fnc(self.data.domain.detach().numpy()).T.squeeze()
                    )
                    ** 2
                )
    ...
    def optimize(self, optimizer_settings_dict=None):
        ...
        if optimizer_settings_dict["optimizer"] == "adam":
            self.opt = torch.optim.Adam(
                sum(self.model.trainable_params.values(), []),
                lr=optimizer_settings_dict["learning_rate"],
            )
        ...
        for i in range(1, self.optimizer_settings_dict["n_iter"] + 1):
            ...
            if i % self.optimizer_settings_dict["update_interval"] == 0:
                self.update(i, loss.detach().numpy())
    ...
    def optimize_optuna(self, optimizer_settings_dict, trial):
        ...
        for i in range(1, self.optimizer_settings_dict["n_iter"] + 1):
            ...
            if (datetime.now() - start_time).total_seconds() > (10 * 60):
                return min(self.loss_values["analytical_loss"])
    ...
    def plot_function_values_over_t(self, ax: plt.Axes = None):
        ...
        if self.model.analytical_fnc != None:
            t = torch.linspace(
                self.model.data.domain_dict["t"][0],
                self.model.data.domain_dict["t"][1],
                1000,
            )
            ax.plot(
                t,
                self.model.analytical_fnc(t).T.squeeze(),
                label="Analytical",
                c="green",
                lw=4,
                alpha=0.5,
