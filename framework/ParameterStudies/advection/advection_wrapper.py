...
def advection_config_and_training_wrapper(trial):
    ...
    learning_rate = trial.suggest_float("learning rate", 0.001, 1.0, log=True)
    optimizer = "lbfgs"
    ...
    boundary_initial_loss_scaling = trial.suggest_float("boundary_loss_scaling", 0.1, 1000.0, log=True)
    boundary_sides_loss_scaling = trial.suggest_float("boundary_loss_scaling", 0.1, 100.0, log=True)
    
    embedding_choice = trial.suggest_categorical("embedding function", ["chebyshev_tower_embedding_rescaled_alternating_2d", "atan_embedding_alternating_2d", "chebyshev_tower_embedding_rescaled_parallel_2d", "chebyshev_tower_embedding_rescaled_sequential_2d"])
    ...
    domain_dict = {"t":[0.001,0.999,t_sampling_points],"x":[-0.999,0.999,x_sampling_points]} #TODO check if include the boundary points
    ...
    solver = framework.Solver(data, model,
                            ...
                            loss_scaling=[1,boundary_sides_loss_scaling,boundary_sides_loss_scaling,boundary_initial_loss_scaling])

    if optimizer == "lbfgs":
        solver_settings_lbfgs = {"optimizer":"lbfgs", "learning_rate":learning_rate, "max_runtime":5, "n_iter":100}
    elif optimizer == "adam":
        solver_settings_adam  = {"optimizer":"adam" , "learning_rate":learning_rate, "max_runtime":5,"n_iter":1000}
    else:
        raise ValueError("Optimizer not supported")

    solver.optimize_optuna(solver_settings_lbfgs)
